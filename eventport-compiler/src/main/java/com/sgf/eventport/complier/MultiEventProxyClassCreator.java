...
public class MultiEventProxyClassCreator implements EventClassCreator{
    ...
    private final static String EVENT_MANAGE_FIELD_NAME = "eventHandlerList";
    private final static String EVENT_HANDLER_FIELD_NAME = "eventHandler";
    ...
    public MultiEventProxyClassCreator(Elements elementUtils, TypeElement classElement, Log log) {
    ...
    public TypeSpec generateJavaCode() {
        ...
        ParameterizedTypeName typeName = ParameterizedTypeName.get(
                ...
                ClassName.get(mPackageName, mEventInterface.getSimpleName().toString()));
    ...
    private MethodSpec createMethod(EventMethod eventMethod,TypeSpec.Builder classBuilder, String className) {
        ...
        ParameterizedTypeName listTypeName = ParameterizedTypeName.get(ClassName.get(List.class),ClassName.get(mPackageName, mEventInterface.getSimpleName().toString()));
        eventObj.append(listTypeName.toString()).append(" ").append(EVENT_MANAGE_FIELD_NAME);
        eventObj.append(" = $T.Companion.getInstance().findEventInterfaceList(").append(EVENT_KEY).append(")");
        ...
        eventManagerMethod.append(EVENT_HANDLER_FIELD_NAME).append(".").append(eventMethod.getMethodName()).append("(");
        // 设置异常
        ...
        methodBuilder.beginControlFlow("for (" + mEventInterface.getSimpleName().toString() + " " + EVENT_HANDLER_FIELD_NAME+ " : " + EVENT_MANAGE_FIELD_NAME +")");
